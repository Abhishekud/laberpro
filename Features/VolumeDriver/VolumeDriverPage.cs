...
namespace LaborPro.Automation.Features.VolumeDriver
{
    public class VolumeDriverPage
    {
        ...
        public static void AddNewVolumeDriverWithGivenInputIfNotExist(Table inputData)
        {
            ...
            var dictionary = Util.ConvertToDictionary(inputData);
            var record = WebDriverUtil.GetWebElementAndScroll(string.Format(VolumeDriverRecord, dictionary["Name"]), WebDriverUtil.NO_WAIT, WebDriverUtil.NO_MESSAGE);
            if (record == null)
            {
        ...
        public static void DeleteVolumeDriverIfExist(string volumeDriverName)
        {
            ...
            var record = WebDriverUtil.GetWebElementAndScroll(string.Format(VolumeDriverRecord, volumeDriverName), WebDriverUtil.NO_WAIT, WebDriverUtil.NO_MESSAGE);
            if (record != null)
            {
